...
- **오차 분석(안전 관점)**: 고장 임박(RUL 0–25) 구간 **RMSE 2.88**로 가장 정확(안전상 중요). 단 **늦은예측 비율**(RUL 과대=고장 놓침)을 별도 추적 → asym 손실/임계로 보정.
- **배포**: PyTorch→**ONNX**(정합오차 0.0) CPU p50 **0.28ms(~10배↑)**, int8 양자화. **드리프트**(PSI/KS) 모니터. **XAI**(Captum IG) 센서 기여도. *(현재 demo 모델 기준 → 그리드서치 best로 갱신 예정)*
